...
export default async function HomePage() {
  const [newPosts, homeDeals, topPicks, homeReviews, homeBlogPreview] = await Promise.all([
    getNewHomePosts(),
    getHomeFeaturedDeals(),
    getTopPicks(),
    getHomeReviews(),
      ...
      <main className="bg-cream text-purple-deep min-h-screen">
        ...
        <section className="px-6 md:px-12 py-8 max-w-[1200px] mx-auto bg-gradient-to-br from-mauve-50 to-pink-200">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-6">
            <div>
              <p className="font-heading italic text-[32px] font-bold mb-3.5 text-purple">
                THE CEDAR METHOD
              </p>
              <p className="text-sm leading-relaxed text-tan-dark text-justify">
                Cedar blocks have kept moths out of closets for generations. Untreated
                cedarwood, left unsanded, releases a natural oil that moths avoid, and lasts for
                years, protects wool and cashmere, works in drawers and hanging bags alike, and
                needs no chemicals, no sprays, and no strong scent left on the clothes
                themselves.
              </p>
            </div>
            <ImageSlot label="Cedar blocks in a wardrobe drawer" className="w-full h-[260px]" tone="pink" />
          </div>
          <div className="grid md:grid-cols-2 gap-8">
            <ImageSlot label="Garment steamer product photo" className="w-full h-[220px]" tone="mauve" />
            <div>
              <p className="font-heading italic text-[19px] font-bold mb-3 text-purple">
                GARMENT STEAMER PRO
              </p>
              <ul className="text-[13.5px] leading-relaxed text-tan-dark pl-4.5 list-disc">
                <li>Fastest heat-up in our test, ready in under a minute for a full closet of steaming.</li>
              </ul>
            </div>
          </div>
        ...
        <Reveal>
          <section className="grid lg:grid-cols-[2fr_1fr] gap-10 px-6 md:px-12 py-16 max-w-[1400px] mx-auto">
            ...
            <aside className="bg-gradient-to-br from-pink-200 to-mauve-50 rounded-2xl p-5.5">
              ...
              {homeDeals.map((d) => (
                <div key={d.id} className="flex gap-3.5 py-3.5 border-b border-pink-100">
        ...
        <Reveal>
          <section id="top-picks" className="px-6 md:px-12 pb-20 max-w-[1400px] mx-auto">
            ...
            {topPicks.map((pick) => (
              <div
                key={pick.id}
                className="flex items-center gap-5 px-4.5 py-4 mb-2.5 rounded-2xl bg-gradient-to-r from-pink-200 to-mauve-50"
              >
                ...
                <div className="flex-1">
                  <div className="font-semibold text-[15px]">{pick.name}</div>
        ...
        <Reveal>
          <section id="guides" className="bg-mauve-100 py-22 px-6 md:px-12">
            <div className="grid md:grid-cols-2 gap-12 items-center max-w-[1400px] mx-auto">
              <ImageSlot
                label="Jewelry box buying guide photo"
              ...
              <div>
                ...
                <h2 className="font-heading text-[30px] leading-tight mb-4 text-purple-deep">
                  How to Choose a Jewelry Box That Actually Protects What&apos;s Inside
                </h2>
                <p className="text-[15px] leading-relaxed text-tan-dark mb-5.5 max-w-[460px]">
                  Anti-tarnish lining, ring rolls versus slots, and why the hinge matters more
                  than the finish. Everything to check before you buy.
                </p>
                <Link
                  href="/blog/jewelry-box-buying-guide"
                  className="inline-block bg-purple-deep text-white px-6.5 py-3 rounded-xl font-semibold text-sm shadow-[0_4px_14px_rgba(46,37,54,0.2)]"
                >
                  Read the Guide
                </Link>
              </div>
