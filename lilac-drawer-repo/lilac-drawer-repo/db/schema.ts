...
/** A comment on a community post. Supports threaded replies via parentId. */
...
/** Follows between users — one row per (follower, following). */
export const follows = pgTable(
  ...
  (table) => [uniqueIndex("follows_follower_following_unique").on(table.followerId, table.followingId)],
);
 ...
 * Allows both authenticated users and guest visitors to subscribe.
 */
