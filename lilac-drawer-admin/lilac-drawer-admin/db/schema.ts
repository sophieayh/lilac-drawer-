...
// NOTE — kept in sync by hand with lilac-drawer-repo/db/schema.ts (the
// public site). Both apps are separate Next.js projects pointed at the
// *same* Postgres database, so this file and the public site's copy must
// always describe the exact same tables/columns. The public site owns
// migrations (`npm run db:generate` / `db:migrate` over there) — after a
// schema change ships on that side, copy the same change into this file
